        ...
        $datacount = count($data);
            ...
            if($datacount>0){
                ...
                for($i = 0; $i < $datacount; $i++){    
                    ...
                    $idpat = $data[$i]['id'];
                    $message[$i] = $prenom.' '.$nom.'<br>résidant '.$adresse.' '.$ville.' '.$cp.'<br>Tel : '.$numtel.'<br>@Mail : '.'<a href="mailto:?to='.$mail.'">'.$mail.'</a>'.' '.'<br><a href="/phpMed/connexion/admin/profil/fiche?id='.$i.'" class="btn btn-outline-dark btn-sm">Fiche patient</a>';
