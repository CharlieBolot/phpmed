...
$prenom = $rech[1];
    ...
    $message=[];
        ...
        if($data['id']>0){
            ...
            $message[0]=$prenom.' '.$nom.'<br>résidant '.$adresse.' '.$ville.' '.$cp.'<br>Tel : '.$numtel.'<br>@Mail : '.'<a href="mailto:?to='.$mail.'">'.$mail.'</a>'.' '.'<br><a href="/phpMed/connexion/admin/profil/fiche?id=0" class="btn btn-outline-dark btn-sm">Fiche patient</a>';
            //$fiche = [$data];
